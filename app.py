import streamlit as st
import requests
import json
import time
from datetime import datetime
import re
import base64

# ==========================================
# 1. 配置與版本
# ==========================================
VERSION = "v1.1.0"
st.set_page_config(page_title=f"Invoice API Tester {VERSION}", page_icon="🧾", layout="wide")

st.title(f"🧾 電子發票 API 測試 - 版本 {VERSION}")
st.success("已恢復 JSON 編輯區，並保持發票號碼自動命名功能。")

# ==========================================
# 2. 狀態管理
# ==========================================
if 'logs' not in st.session_state: st.session_state.logs = []
if 'files' not in st.session_state: st.session_state.files = []
if 'sent' not in st.session_state: st.session_state.sent = 0
if 'received' not in st.session_state: st.session_state.received = 0

# ==========================================
# 3. 側邊欄配置
# ==========================================
with st.sidebar:
    st.header("⚙️ 配置")
    target_url = st.text_input("API URL", value="https://gw-ns-service.tweinv.com:443/api/v1/pdf/invoice")
    sec_interval = st.number_input("發送間隔 (秒)", min_value=0.1, value=1.0)
    total_rounds = st.number_input("總發送次數", min_value=1, value=5)

    if st.button("🗑️ 清除所有紀錄"):
        st.session_state.update({"sent": 0, "received": 0, "logs": [], "files": []})
        st.rerun()

# ==========================================
# 4. JSON 編輯區域 (恢復顯示)
# ==========================================
default_correct_data = {
    "url": "",
    "payload": {
        "xml": "",
        "filename": "C0401-QS26376438-172882.xml",
        "checksum": "",
        "documentStatus": 2,
        "extramemo": ""
    },
    "attribute": {
        "printerName": "Sharp",
        "printerType": "GENERAL",
        "printerKey": "uuid",
        "lineSpacing": 1,
        "printDetail": False,
        "printContact": False,
        "reprint": False,
        "uploadDocument": True
    }
}

json_input = st.text_area("🔧 JSON Payload 編輯區：", value=json.dumps(default_correct_data, indent=2), height=400)

# ==========================================
# 5. 發送邏輯
# ==========================================
if st.button(f"🚀 開始執行測試", use_container_width=True):
    try:
        # 解析輸入內容
        current_payload = json.loads(json_input)
        
        # 嘗試從 XML 中動態抓取發票號碼供命名使用
        try:
            xml_str = base64.b64decode(current_payload["payload"]["xml"]).decode('utf-8')
            inv_match = re.search(r'<InvoiceNumber>(.*?)</InvoiceNumber>', xml_str)
            inv_no = inv_match.group(1) if inv_match else "Unknown"
        except:
            inv_no = "Invoice"

        compact_body = json.dumps(current_payload, separators=(',', ':'))
        headers = {"Content-Type": "text/plain", "User-Agent": "Mozilla/5.0"}

        for i in range(int(total_rounds)):
            current_round = i + 1
            st.session_state.sent += 1
            timestamp = datetime.now().strftime("%H%M%S")
            
            try:
                r = requests.post(target_url, data=compact_body.encode('utf-8'), headers=headers, timeout=20)
                
                if r.status_code == 200:
                    st.session_state.received += 1
                    # 命名：發票號碼_序列_時間戳.pdf
                    file_name = f"{inv_no}_{current_round}_{timestamp}.pdf"
                    
                    st.session_state.files.append({"name": file_name, "content": r.content})
                    st.session_state.logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ 成功: {file_name}")
                else:
                    st.session_state.logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] ❌ 失敗 ({r.status_code})")
            
            except Exception as e:
                st.session_state.logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ 異常: {str(e)}")

            if i < total_rounds - 1: time.sleep(sec_interval)
        
        st.rerun()

    except json.JSONDecodeError:
        st.error("JSON 格式錯誤，請檢查括號或逗號是否正確。")

# ==========================================
# 6. 下載區與日誌
# ==========================================
st.divider()
c1, c2 = st.columns(2)
with c1:
    st.subheader(f"📂 下載區 (已獲取 {len(st.session_state.files)} 份)")
    if st.session_state.files:
        grid = st.columns(2)
        for idx, f in enumerate(st.session_state.files):
            with grid[idx % 2]:
                st.download_button(label=f"⬇️ {f['name']}", data=f['content'], file_name=f['name'], key=f"dl_{idx}")
with c2:
    st.subheader("📜 執行日誌")
    st.text_area("Log", "\n".join(reversed(st.session_state.logs)), height=350)
